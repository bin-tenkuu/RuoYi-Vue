...
public class SysDictTypeServiceImpl implements ISysDictTypeService
{
    ...
    public void deleteDictTypeByIds(Long[] dictIds)
    {
        for (Long dictId : dictIds)
        {
            ...
            {
                throw new ResultException(String.format("%1$s已分配,不能删除", dictType.getDictName()));
            }
            dictTypeMapper.deleteDictTypeById(dictId);
            DICT_CACHE.remove(dictType.getDictType());
        }
    }
    ...
    public void loadingDictCache()
    {
        ...
        {
            DICT_CACHE.put(entry.getKey(), entry.getValue().stream().sorted(Comparator.comparing(SysDictData::getDictSort)).collect(Collectors.toList()));
        }
    }
